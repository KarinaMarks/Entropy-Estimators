...
\item Strong Consistency (Asymptotic Normality) %TODO - look at the second/third line of this? What should it be ask leonenko!
\begin{align}
\sqrt{N}(\hat{H}_{N, k} - H) &\xrightarrow{d} N(0, \sigma^2) \\
N \mathbb{E}{(\hat{H}_{N, k} - H)^2} &\xrightarrow{} \sigma^2 \\ 
\mathbb{E}{(\hat{H}_{N, k} - H)^2} &= \frac{\sigma^2}{N} \left( 1 + O(N) \right) \\ \nonumber
\end{align}
...
I created a package with the functions needed to created the Kozachenko-Leonenko entropy estimator (KLEE), and then used this package to run simulations on samples from different statistical distributions to create the results in sections \ref{Uniform_results}, \ref{Normal_results} and \ref{Expo_results}.
