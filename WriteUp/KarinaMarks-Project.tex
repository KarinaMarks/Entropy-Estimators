...
{\large \today}\\[2cm] % Date as today

\includegraphics[width=0.4\textwidth]{logo.png}\\ % Include university logo
 
\vfill 
\end{titlepage}

\begin{abstract}
The Kozachenko-Leonenko (K-L) estimator, $\hat{H}_{N, k}$, \cite{paper1} (1987) is used to estimate the entropy of a sample made up of $N$ vectors $x \in \mathbb{R}^d$, based upon the $k$th nearest neighbour sample spacings. There is extensive theoretical research into the conditions under which the asymptotic unbias and consistency of this estimator holds \cite{paper4, paper3, paper1, paper2}. However, this paper focuses on the investigation of how the estimator works analytically, by exploring two main research questions: ``How does the bias of the estimator act, $Bias|\hat{H}_{N, k}| = O \left( \frac{1}{N^{a}} \right)$ \cite{paper3} or $Bias|\hat{H}_{N, k}| = O\left( \left( \frac{k}{N} \right)^{a} \right)$ \cite{paper4}?'' and ``What is the optimal value to choose for k in terms of reducing the bias?'' This research utilised a programming software, R \cite{code1} (Appendix \ref{appendix_code}), to run simulations to generate a large number of samples, of different sizes $N$, from the 1-dimensional uniform, normal and exponential distributions, for the K-L estimator to be calculated from, with varying $k$. Then, the findings were summarised (Appendix \ref{appendix_data}), linear regression analysis and other techniques were conducted to study the research questions, and lastly, the interpretations of the data were written up in LaTeX. In general, the investigation found varying results between distributions and sample sizes, thus creating difficulty in making any conclusive answers. One irrefutable result, found for all distributions and values of $k$, was that $Bias|\hat{H}_{N, k}| = \frac{c}{N^a}$, with $a \geq 0.49$, where the only discrepancies found being in the analysis of the behaviour of $c$. For samples from the uniform and normal distributions, the analysis suggests that bias of the estimator acts of $O\left( \left( \frac{k}{N} \right)^{a} \right)$ ($c = O(k ^a) $). Conversely, for samples from the exponential distribution, the analysis implied a relation of the other form, $O \left( \frac{1}{N^{a}} \right)$ ($c$ is constant). Moreover, some conclusions have been drawn about the optimal choice of $k$ in reducing the bias for a sample size $N$; yet, there was variability for all $N$, giving a range of values of $k$ - not one choice that always works.
\end{abstract}
...
\begin{align}
\sqrt{N}(H_{N} - H(f)) &\xrightarrow{d} N(0, \sigma^2) \\
\mathbb{E}{(H_{N} - H(f))^2} &= \frac{\sigma^2}{N} \left( 1 + O\left( \frac{1}{N} \right) \right) \\ \nonumber
\end{align}
...
The Kozachenko-Leonenko (K-L) estimator was first introduced by L.Kozachenko and N.Leonenko, in 1987, in the article \textit{Sample Estimate of the Entropy of a Random Vector}, in the journal \textit{Problems of Information Transmission} \cite{paper1}. Using the nearest neighbour method, they created a simple estimator for the Shannon entropy of an absolutely continuous random vector from a independent sample of observations. They then established conditions under which asymptotic unbiasedness and weak consistency ($\hat{H}_{N, k} \xrightarrow{p} H$) hold.
...
\end{equation}
where $c(d) = \frac{\pi^{\frac{d}{2}}}{\Gamma ( \frac{d}{2} + 1 )}$ is the volume of the d-dimensional unit ball, the Euler constant is $\log (\gamma) = \log \left( \exp \left[ - \int_{0}^{\infty} e^{-t} \log(t) dt \right] \right) =  - \int_{0}^{\infty} e^{-t} \log(t) dt = -\Psi(1)$ and $\bar{\rho} = \left[ \prod_{i=1}^{N} \rho_{i} \right]^{\frac{1}{N}}$, with $\rho_{i}$ the nearest neighbour distance from $X_{i}$ to another member of the sample $X_{j}$, $i \neq j$. 
...
By the above, we can now say that the K-L estimator, $\hat{H}_{N, k}$, that satisfies Conditions \ref{A1}, \ref{A2}, \ref{A3} is a consistent and asymptotically unbiased estimator of exact entropy $H$; thus is a efficient estimator. This is due to using the central limit theorem, on the estimator for entropy $\hat{H}_{N, k}$ \cite{paper6}, which states that:
\begin{equation}
...
\begin{itemize}
\item With a fixed k, by \cite{paper3}, for $a >0.5$ we have:
\begin{equation} \label{fixedkbias}
Bias(\hat{H}_{N, k})= O \left( \frac{1}{N^{a}} \right)
\end{equation}

\item With k depending on N, by \cite{paper4}, for $a >0.5$ we have:
...
\begin{enumerate}
\item Create a summary table of the mean absolute value of the bias of the estimator just observing the sample sizes $N=100, 25000$ and $50000$ for all values of $k$ that satisfy Condition \ref{A3}. I could also at this point consider the variance of the bias at the values of $N$ stated above, for all applicable values of $k$. However, we will find that the $Var|Bias(\hat{H}_{50000, k})| \to 0$ for $k \to 11$, by the definition of the estimator using the nearest neighbour method. Taking a larger $k$ in the nearest neighbour method will produce less varied results. This is because more smoothing takes place for a larger $k$, eventually - if $k$ is made large enough - the output will be constant and the variance negligible regardless of the inputted values. Thus, considering the variance of the bias of the estimator in comparison to $k$ is not necessarily informative. 
...
The $R^2$ is very small for $k \leq 3$ , which suggests that the line is a poor fit to the data; however, due the the standard deviation being $\sigma^2 \approx 1.1$, we cannot say that these lines are poorly fitting; since the majority of the data is within a very small range of the line.
...
From this, we can obviously see that for smaller values of $N$ (i.e - smaller values of $log(N)$), the smallest bias occurs when $k=2$, since this line is the lowest for the data up until $log(N) \approx 9.5$ (i.e. $N \approx 13,000$). For a larger sample size, we cannot accurately see in this graph which line is the best. This motivates us to look at a section of the graph when $9 \leq log(N) \leq 11$ - i.e. $8,000 \leq N \leq 50,000$ - Figure \ref{normal_comparison_graph_zoom}.
...
The only other function needed for the estimator is the digamma function which is defined in base R; thus, we can now define the K-L estimator \texttt{KLEE}. This takes three arguments:
\begin{itemize}
...
\renewcommand{\bibname}{References} % change the name of the bibliography
