...
\begin{itemize}
\item outliers
\item the relationship isn't smooth but does overall tend that way
\item estimator for the expo dist is not asymptotically unbiased - not true since we've shown it satisfies theorems
\end{itemize}
...
TODO... this graph shows... A more detailed comparison will be further explored in section \ref{E_compare_k}.
...
Using the same parameters as before; taking $500$ samples of size $N=100 \to 50,000$ from the exponential distribution, with rate parameter $\lambda =0.5$, and consider the estimator of these samples against the actual value of entropy, equation \ref{exponential_exact}, we get the results in table \ref{expo_k=2_table}. These results are inconclusive in showing the relationship desired between the $|Bias(\hat{H}_{N, 2})|$ and $N$, from Theorem \ref{paper1_T1}. 

Additionally, I have also plotted the regression graph again, this time for $k=2$, the results are shown in Figure \ref{expo_k=1_graph} and the regression coefficients are given by $a_{2} = 0.6480$ and $c_{2} = 0.1482$.
