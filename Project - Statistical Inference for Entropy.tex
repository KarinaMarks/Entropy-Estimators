...
Furthermore, I have considered a plot to represent equation (\ref{logbias}), shown in Figure \ref{expo_k=5_graph}, where I have also found the coefficients of regression to be; $a_{5} =$ and $c_{5} =$. 
...
\begin{equation}
|Bias(\hat{H}_{N, 5})| \approx \frac{}{N^{}}\nonumber
\end{equation}
In comparison to the coefficients from the previous value of $k$, we find that... 
...
\hat{H}_{N, 10} = \frac{1}{N} \sum_{i=1}^{N} log \left[ \frac{2\rho_{(10),i}(N-1)}{e^{\Psi(10)}} \right] \nonumber
\end{equation}
The results for the comparison between this estimator and \ref{exponential_exact} are displayed in table \ref{expo_k=10_table}.
